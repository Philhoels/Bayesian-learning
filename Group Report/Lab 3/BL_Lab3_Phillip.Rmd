...
plot_tace_mu <- ggplot(data = Gibbs_sampling) + 
  ...
  geom_smooth(aes(x = iterations, y = mu),method="auto") +
  ggtitle("Tace of mu")
  # plot sigma2
plot_tace_sigma2 <-ggplot(data = Gibbs_sampling) + 
  geom_line(aes(x = iterations, y = sigma2), color = "#2980b9") + 
  geom_smooth(aes(x = iterations, y = sigma2), method = "auto")+
...
In the first picture you can see the tendacy of $\mu$, which converges very quickly. The starting value is about 22, but after a few iterations it approaches the value of 26-27.
...
plot_data_q1aii <- plot_data_q1aii[1:20,] # test- just take the first x rows
...
plot_mu_vs_sigma_line <- ggplot(plot_data_q1aii, 
  ...
  geom_point(aes(x = plot_data_q1aii$mu[length(plot_data_q1aii$mu)], 
                 ...
                 color = "end"), size = 3) +
  ggtitle("mu vs sigma2 - line 20 points") + ylab("sigma2") + xlab("mu") +
...
plot_data_q1c2 <-  rnorm(n = nDraws, mean = Gibbs_sampling$mu[i], 
